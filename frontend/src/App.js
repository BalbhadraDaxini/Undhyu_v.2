...
const API_BASE_URL = process.env.REACT_APP_BACKEND_URL || 'https://fashion-bazaar-3.preview.emergentagent.com';
...
function App() {
  ...
  const proceedWithPayment = async () => {
    ...
    try {
      const totalAmount = Math.round(pendingCart.reduce((total, item) => total + (item.price * item.quantity), 0) * 100);
